...
namespace Snake
{
...

    {
        ...
        public int GameOverCheck(Queue<Position> snakeElements, Position snakeNewHead, int negativePoints, List<Position> obstacles)
        {
            ...
            if (snakeElements.Contains(snakeNewHead) || obstacles.Contains(snakeNewHead))
            {
                ...
                Console.ForegroundColor = ConsoleColor.Red;//Text color for game over
                                                           // Console.WriteLine("{ 1,20}","Game over!");//The text which user will view when game is over
        ...
        public void SavePointsToFile(int userPoints, string user_name)
        {
            ...
            try
            {
                if (!File.Exists(filePath))
                {
                    ...
                    File.WriteAllText(filePath, userPoints.ToString() + user_name);
                }
                else
                {
                    File.AppendAllText(filePath, userPoints.ToString() + user_name);
                }
            }
        ...
        static void Main(string[] args)
        {
            ...
            int foodDissapearTime = 0; //food dissappears after 15 second 
            double sleepTime = 0;
